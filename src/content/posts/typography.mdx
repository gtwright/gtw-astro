...
The [design system post](/posts/design-system) introduced the prose measure as a layout constraint — a `max-width: 65ch` that sits inside the `max-w-3xl` page container, centered with `margin-left: auto; margin-right: auto`. The container frames the full page; the prose centers and narrows the body text for reading — the same pattern you'll find on Medium or the New York Times. Here's why that specific value.
