...
Since the project is on Astro v6 beta, the adapter needs to match. The stable release targets Astro v5. The [Cloudflare adapter docs](https://v6.docs.astro.build/en/guides/integrations-guide/cloudflare/) cover the full configuration reference.
...
I'm skipping over the Cloudflare account setup and Wrangler authentication here. Both are straightforward and [well-documented](https://developers.cloudflare.com/workers/get-started/guide/).
...
The [next post](/posts/setting-up-the-workshop) covers what I did before writing any components or content: setting up the project documentation that shapes every AI-assisted building session.
