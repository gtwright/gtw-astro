...
This post describes the hand-rolled SEO component I built at launch. The site has since migrated to [`@jdevalk/astro-seo-graph`](https://github.com/jdevalk/seo-graph) — a linked JSON-LD graph with build-time validation, IndexNow, and agent discovery features. The principles here still hold; the implementation has evolved.
</Callout>

The [typography post](/posts/typography/) covered the typographic details that make prose readable — spacing, sizing, and visual hierarchy inside the `<Prose>` component. That work is for human readers. This post is about the features that serve a different audience: search engines, feed readers, social platforms, and web crawlers.
