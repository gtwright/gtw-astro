...
A series documenting the process of building a personal blog with Astro, from intent to production. Phase 0 covers the thinking — why build this, what principles guide it, and which tools to use. The pre-launch posts are grouped thematically, each mapping to a retroactive PR. Post-launch, each post corresponds to a real PR.
...
## Pre-Launch (Thematic grouping, retroactive PRs)

Work in this section happened iteratively, not linearly. Posts are organized by theme. Each PR represents the final state of that theme at launch, even if the work was spread across multiple sessions.

### Part 1: First Boot — Scaffolding the Project

**PR scope:** Project scaffolding, initial config, first successful build.

- `npm create astro`, TypeScript strict mode, initial `astro.config.mjs`.
...
**PR scope:** CLAUDE.md, .cursorrules, README, initial working documents.
...
**PR scope:** Tailwind v4, custom theme tokens, variable fonts, global styles.
...
**PR scope:** Prose component, fine-tuned styles for all Markdown elements.
...
**PR scope:** Content schema, MDX setup, blog listing, dynamic post routes, PostCard component.
...
**PR scope:** Base layout, Header, Footer, site constants.
...
**PR scope:** SEO component, RSS feed, sitemap integration, Open Graph defaults, robots.txt.
...
**PR scope:** Image optimization setup, responsive images in content and components.
...
**PR scope:** ClientRouter, transition directives, WaveBackground component, diagnostics.
...
**PR scope:** 404 page, remove test/diagnostic files, real domain, production deployment.
...
## Post-Launch (Real PRs, real posts)

From here on, each post maps to a real PR created as the work happens.

### Part 11: Dark Mode

**PR scope:** Color scheme toggle, CSS custom properties, persistence.
...
**PR scope:** Initial audit, fixes for low-hanging fruit, tracking setup.
...
**PR scope:** GitHub Actions, type checking, build verification, deployment automation.
...
**PR scope:** Editorial tracking documents, draft workflow, publishing automation.
...
**PR scope:** Reading time calculation, header anchor links, other polish.
...
**PR scope:** Tag pages, tag listing, category system.
...
**PR scope:** Client-side search integration.
...
**PR scope:** Privacy-respecting analytics integration.
...
- **Phase 0 has no PRs.** These posts are pure framing — decisions and principles documented before any code is written. They establish the "why" that the rest of the series builds on.
- **Pre-launch posts are retroactive.** The work wasn't done in neat PR-sized chunks. We built iteratively, then grouped by theme. The git history for pre-launch is an edited narrative — accurate in substance, tidied in sequence.
- **Post-launch posts are real-time.** Each maps to a PR created as the work happens.
